...
<template>
      ...
      <div>
        <div class="flex text-center">
          ...
          <div class="flex justify-between w-full">
            ...
            <button class="underline text-red-700" @click="openRewiewModal">
              Edit
            </button>
          </div>
        </div>
        <div class="flex h-[225px]">
          <img class="w-[175px]" src="..." />
          <div class="p-8">
            <RatingIcon />
          </div>
          <div class="w-2/3 p-10 pb-10">
            It can be said thatno one prefers a life in darkness to a life with
            a bright future. Sometimes it is unavoidable that darkness comes
            into our lives, however, we know that when tomorrow comes, all
            darkness will disappear. With the presence of daylight, we will gain
            hope and courage.
          </div>
        </div>
      </div>
  ...
  <reviewPage :open="showReview" @close="showReview = !showReview" />
</template>
